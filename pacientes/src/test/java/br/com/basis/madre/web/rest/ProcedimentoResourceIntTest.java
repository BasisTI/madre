...
public class ProcedimentoResourceIntTest {
    ...
    public void setup() {
        ...
        final ProcedimentoResource procedimentoResource = new ProcedimentoResource(procedimentoService);
    ...
    public void searchProcedimento() throws Exception {
        // Initialize the database
        procedimentoRepository.saveAndFlush(procedimento);
        when(mockProcedimentoSearchRepository.search(queryStringQuery("id:" + procedimento.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(procedimento), PageRequest.of(0, 1), 1));
        // Search the procedimento
        restProcedimentoMockMvc.perform(get("/api/_search/procedimentos?query=id:" + procedimento.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(procedimento.getId().intValue())))
            .andExpect(jsonPath("$.[*].codigo").value(hasItem(DEFAULT_CODIGO.intValue())))
