...
public class ModalidadeAssistencialResourceIT {
    ...
    public void setup() {
        ...
        final ModalidadeAssistencialResource modalidadeAssistencialResource = new ModalidadeAssistencialResource(modalidadeAssistencialService);
        this.restModalidadeAssistencialMockMvc = MockMvcBuilders.standaloneSetup(modalidadeAssistencialResource)
    ...
    public void createModalidadeAssistencial() throws Exception {
        ...
        ModalidadeAssistencialDTO modalidadeAssistencialDTO = modalidadeAssistencialMapper.toDto(modalidadeAssistencial);
        ...
        List<ModalidadeAssistencial> modalidadeAssistencialList = modalidadeAssistencialRepository.findAll();
        assertThat(modalidadeAssistencialList).hasSize(databaseSizeBeforeCreate + 1);
        ModalidadeAssistencial testModalidadeAssistencial = modalidadeAssistencialList.get(modalidadeAssistencialList.size() - 1);
        assertThat(testModalidadeAssistencial.getNome()).isEqualTo(DEFAULT_NOME);

        // Validate the ModalidadeAssistencial in Elasticsearch
        verify(mockModalidadeAssistencialSearchRepository, times(1)).save(testModalidadeAssistencial);
    }
    ...
    public void getAllModalidadeAssistencials() throws Exception {
        ...
        restModalidadeAssistencialMockMvc.perform(get("/api/modalidade-assistencials?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(modalidadeAssistencial.getId().intValue())))
    ...
    public void getModalidadeAssistencial() throws Exception {
        ...
        restModalidadeAssistencialMockMvc.perform(get("/api/modalidade-assistencials/{id}", modalidadeAssistencial.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getNonExistingModalidadeAssistencial() throws Exception {
        // Get the modalidadeAssistencial
        restModalidadeAssistencialMockMvc.perform(get("/api/modalidade-assistencials/{id}", Long.MAX_VALUE))
    ...
    public void updateModalidadeAssistencial() throws Exception {
        ...
        ModalidadeAssistencial updatedModalidadeAssistencial = modalidadeAssistencialRepository.findById(modalidadeAssistencial.getId()).get();
        ...
        updatedModalidadeAssistencial
            .nome(UPDATED_NOME);
        ModalidadeAssistencialDTO modalidadeAssistencialDTO = modalidadeAssistencialMapper.toDto(updatedModalidadeAssistencial);
    ...
    public void deleteModalidadeAssistencial() throws Exception {
        ...
        restModalidadeAssistencialMockMvc.perform(delete("/api/modalidade-assistencials/{id}", modalidadeAssistencial.getId())
        ...
        // Validate the ModalidadeAssistencial in Elasticsearch
        verify(mockModalidadeAssistencialSearchRepository, times(1)).deleteById(modalidadeAssistencial.getId());
    }
    ...
    public void searchModalidadeAssistencial() throws Exception {
        // Initialize the database
        modalidadeAssistencialRepository.saveAndFlush(modalidadeAssistencial);
        when(mockModalidadeAssistencialSearchRepository.search(queryStringQuery("id:" + modalidadeAssistencial.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(modalidadeAssistencial), PageRequest.of(0, 1), 1));
        // Search the modalidadeAssistencial
        restModalidadeAssistencialMockMvc.perform(get("/api/_search/modalidade-assistencials?query=id:" + modalidadeAssistencial.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(modalidadeAssistencial.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)));
