...
public class OrigemDaInternacaoResourceIT {
    ...
    public void setup() {
        ...
        final OrigemDaInternacaoResource origemDaInternacaoResource = new OrigemDaInternacaoResource(origemDaInternacaoService);
        this.restOrigemDaInternacaoMockMvc = MockMvcBuilders.standaloneSetup(origemDaInternacaoResource)
    ...
    public void createOrigemDaInternacao() throws Exception {
        ...
        OrigemDaInternacaoDTO origemDaInternacaoDTO = origemDaInternacaoMapper.toDto(origemDaInternacao);
        ...
        OrigemDaInternacao testOrigemDaInternacao = origemDaInternacaoList.get(origemDaInternacaoList.size() - 1);
    ...
    public void getOrigemDaInternacao() throws Exception {
        ...
        restOrigemDaInternacaoMockMvc.perform(get("/api/origem-da-internacaos/{id}", origemDaInternacao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getNonExistingOrigemDaInternacao() throws Exception {
        // Get the origemDaInternacao
        restOrigemDaInternacaoMockMvc.perform(get("/api/origem-da-internacaos/{id}", Long.MAX_VALUE))
    ...
    public void updateOrigemDaInternacao() throws Exception {
        ...
        OrigemDaInternacao updatedOrigemDaInternacao = origemDaInternacaoRepository.findById(origemDaInternacao.getId()).get();
        ...
        updatedOrigemDaInternacao
            .nome(UPDATED_NOME);
        OrigemDaInternacaoDTO origemDaInternacaoDTO = origemDaInternacaoMapper.toDto(updatedOrigemDaInternacao);
    ...
    public void deleteOrigemDaInternacao() throws Exception {
        ...
        restOrigemDaInternacaoMockMvc.perform(delete("/api/origem-da-internacaos/{id}", origemDaInternacao.getId())
        ...
        // Validate the OrigemDaInternacao in Elasticsearch
        verify(mockOrigemDaInternacaoSearchRepository, times(1)).deleteById(origemDaInternacao.getId());
    }
    ...
    public void searchOrigemDaInternacao() throws Exception {
        // Initialize the database
        origemDaInternacaoRepository.saveAndFlush(origemDaInternacao);
        when(mockOrigemDaInternacaoSearchRepository.search(queryStringQuery("id:" + origemDaInternacao.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(origemDaInternacao), PageRequest.of(0, 1), 1));
        // Search the origemDaInternacao
        restOrigemDaInternacaoMockMvc.perform(get("/api/_search/origem-da-internacaos?query=id:" + origemDaInternacao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(origemDaInternacao.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)));
