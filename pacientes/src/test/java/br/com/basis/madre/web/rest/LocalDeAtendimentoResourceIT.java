...
public class LocalDeAtendimentoResourceIT {
    ...
    public void setup() {
        ...
        final LocalDeAtendimentoResource localDeAtendimentoResource = new LocalDeAtendimentoResource(localDeAtendimentoService);
        this.restLocalDeAtendimentoMockMvc = MockMvcBuilders.standaloneSetup(localDeAtendimentoResource)
    ...
    public void createLocalDeAtendimento() throws Exception {
        ...
        LocalDeAtendimentoDTO localDeAtendimentoDTO = localDeAtendimentoMapper.toDto(localDeAtendimento);
        ...
        LocalDeAtendimento testLocalDeAtendimento = localDeAtendimentoList.get(localDeAtendimentoList.size() - 1);
    ...
    public void getLocalDeAtendimento() throws Exception {
        ...
        restLocalDeAtendimentoMockMvc.perform(get("/api/local-de-atendimentos/{id}", localDeAtendimento.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void getNonExistingLocalDeAtendimento() throws Exception {
        // Get the localDeAtendimento
        restLocalDeAtendimentoMockMvc.perform(get("/api/local-de-atendimentos/{id}", Long.MAX_VALUE))
    ...
    public void updateLocalDeAtendimento() throws Exception {
        ...
        LocalDeAtendimento updatedLocalDeAtendimento = localDeAtendimentoRepository.findById(localDeAtendimento.getId()).get();
        ...
        updatedLocalDeAtendimento
            .nome(UPDATED_NOME);
        LocalDeAtendimentoDTO localDeAtendimentoDTO = localDeAtendimentoMapper.toDto(updatedLocalDeAtendimento);
    ...
    public void deleteLocalDeAtendimento() throws Exception {
        ...
        restLocalDeAtendimentoMockMvc.perform(delete("/api/local-de-atendimentos/{id}", localDeAtendimento.getId())
        ...
        // Validate the LocalDeAtendimento in Elasticsearch
        verify(mockLocalDeAtendimentoSearchRepository, times(1)).deleteById(localDeAtendimento.getId());
    }
    ...
    public void searchLocalDeAtendimento() throws Exception {
        // Initialize the database
        localDeAtendimentoRepository.saveAndFlush(localDeAtendimento);
        when(mockLocalDeAtendimentoSearchRepository.search(queryStringQuery("id:" + localDeAtendimento.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(localDeAtendimento), PageRequest.of(0, 1), 1));
        // Search the localDeAtendimento
        restLocalDeAtendimentoMockMvc.perform(get("/api/_search/local-de-atendimentos?query=id:" + localDeAtendimento.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(localDeAtendimento.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)));
