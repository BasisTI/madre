...
public class ProcedenciaResourceIT {
    ...
    public void searchProcedencia() throws Exception {
        // Initialize the database
        procedenciaRepository.saveAndFlush(procedencia);
        when(mockProcedenciaSearchRepository.search(queryStringQuery("id:" + procedencia.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(procedencia), PageRequest.of(0, 1), 1));
        // Search the procedencia
        restProcedenciaMockMvc.perform(get("/api/_search/procedencias?query=id:" + procedencia.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(procedencia.getId().intValue())))
            .andExpect(jsonPath("$.[*].descricao").value(hasItem(DEFAULT_DESCRICAO)));
