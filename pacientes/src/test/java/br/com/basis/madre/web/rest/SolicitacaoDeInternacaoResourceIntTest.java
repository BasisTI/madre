...
/**
 * Test class for the SolicitacaoDeInternacaoResource REST controller.
 *
 * @see SolicitacaoDeInternacaoResource
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = PacientesApp.class)
public class SolicitacaoDeInternacaoResourceIntTest {

    private static final LocalDate DEFAULT_DATA_PROVAVEL_DA_INTERNACAO = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_DATA_PROVAVEL_DA_INTERNACAO = LocalDate.now(ZoneId.systemDefault());

    private static final LocalDate DEFAULT_DATA_PROVAVEL_DA_CIRURGIA = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_DATA_PROVAVEL_DA_CIRURGIA = LocalDate.now(ZoneId.systemDefault());
    ...
    public void setup() {
        ...
        final SolicitacaoDeInternacaoResource solicitacaoDeInternacaoResource = new SolicitacaoDeInternacaoResource(solicitacaoDeInternacaoService);
        this.restSolicitacaoDeInternacaoMockMvc = MockMvcBuilders.standaloneSetup(solicitacaoDeInternacaoResource)
    ...
    public static SolicitacaoDeInternacao createEntity(EntityManager em) {
        SolicitacaoDeInternacao solicitacaoDeInternacao = new SolicitacaoDeInternacao()
            ...
            .principaisResultadosProvasDiagnosticas(DEFAULT_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS);
    ...
    public void createSolicitacaoDeInternacao() throws Exception {
        ...
        SolicitacaoDeInternacaoDTO solicitacaoDeInternacaoDTO = solicitacaoDeInternacaoMapper.toDto(solicitacaoDeInternacao);
        ...
        List<SolicitacaoDeInternacao> solicitacaoDeInternacaoList = solicitacaoDeInternacaoRepository.findAll();
        assertThat(solicitacaoDeInternacaoList).hasSize(databaseSizeBeforeCreate + 1);
        SolicitacaoDeInternacao testSolicitacaoDeInternacao = solicitacaoDeInternacaoList.get(solicitacaoDeInternacaoList.size() - 1);
        assertThat(testSolicitacaoDeInternacao.getDataProvavelDaInternacao()).isEqualTo(DEFAULT_DATA_PROVAVEL_DA_INTERNACAO);
        assertThat(testSolicitacaoDeInternacao.getDataProvavelDaCirurgia()).isEqualTo(DEFAULT_DATA_PROVAVEL_DA_CIRURGIA);
        assertThat(testSolicitacaoDeInternacao.getPrioridade()).isEqualTo(DEFAULT_PRIORIDADE);
        assertThat(testSolicitacaoDeInternacao.getPrincipaisSinaisESintomasClinicos()).isEqualTo(DEFAULT_PRINCIPAIS_SINAIS_E_SINTOMAS_CLINICOS);
        assertThat(testSolicitacaoDeInternacao.getCondicoesQueJustificamInternacao()).isEqualTo(DEFAULT_CONDICOES_QUE_JUSTIFICAM_INTERNACAO);
        assertThat(testSolicitacaoDeInternacao.getPrincipaisResultadosProvasDiagnosticas()).isEqualTo(DEFAULT_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS);

        // Validate the SolicitacaoDeInternacao in Elasticsearch
        verify(mockSolicitacaoDeInternacaoSearchRepository, times(1)).save(testSolicitacaoDeInternacao);
    }
    ...
    public void getAllSolicitacaoDeInternacaos() throws Exception {
        ...
        restSolicitacaoDeInternacaoMockMvc.perform(get("/api/solicitacao-de-internacaos?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(solicitacaoDeInternacao.getId().intValue())))
            .andExpect(jsonPath("$.[*].dataProvavelDaInternacao").value(hasItem(DEFAULT_DATA_PROVAVEL_DA_INTERNACAO.toString())))
            .andExpect(jsonPath("$.[*].dataProvavelDaCirurgia").value(hasItem(DEFAULT_DATA_PROVAVEL_DA_CIRURGIA.toString())))
            .andExpect(jsonPath("$.[*].prioridade").value(hasItem(DEFAULT_PRIORIDADE.toString())))
            .andExpect(jsonPath("$.[*].principaisSinaisESintomasClinicos").value(hasItem(DEFAULT_PRINCIPAIS_SINAIS_E_SINTOMAS_CLINICOS.toString())))
            .andExpect(jsonPath("$.[*].condicoesQueJustificamInternacao").value(hasItem(DEFAULT_CONDICOES_QUE_JUSTIFICAM_INTERNACAO.toString())))
            .andExpect(jsonPath("$.[*].principaisResultadosProvasDiagnosticas").value(hasItem(DEFAULT_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS.toString())));
    }
    ...
    public void getSolicitacaoDeInternacao() throws Exception {
        ...
        restSolicitacaoDeInternacaoMockMvc.perform(get("/api/solicitacao-de-internacaos/{id}", solicitacaoDeInternacao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(solicitacaoDeInternacao.getId().intValue()))
            .andExpect(jsonPath("$.dataProvavelDaInternacao").value(DEFAULT_DATA_PROVAVEL_DA_INTERNACAO.toString()))
            .andExpect(jsonPath("$.dataProvavelDaCirurgia").value(DEFAULT_DATA_PROVAVEL_DA_CIRURGIA.toString()))
            .andExpect(jsonPath("$.prioridade").value(DEFAULT_PRIORIDADE.toString()))
            .andExpect(jsonPath("$.principaisSinaisESintomasClinicos").value(DEFAULT_PRINCIPAIS_SINAIS_E_SINTOMAS_CLINICOS.toString()))
            .andExpect(jsonPath("$.condicoesQueJustificamInternacao").value(DEFAULT_CONDICOES_QUE_JUSTIFICAM_INTERNACAO.toString()))
            .andExpect(jsonPath("$.principaisResultadosProvasDiagnosticas").value(DEFAULT_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS.toString()));
    }
    ...
    public void getNonExistingSolicitacaoDeInternacao() throws Exception {
        // Get the solicitacaoDeInternacao
        restSolicitacaoDeInternacaoMockMvc.perform(get("/api/solicitacao-de-internacaos/{id}", Long.MAX_VALUE))
    ...
    public void updateSolicitacaoDeInternacao() throws Exception {
        ...
        SolicitacaoDeInternacao updatedSolicitacaoDeInternacao = solicitacaoDeInternacaoRepository.findById(solicitacaoDeInternacao.getId()).get();
        ...
        updatedSolicitacaoDeInternacao
            ...
            .principaisResultadosProvasDiagnosticas(UPDATED_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS);
        SolicitacaoDeInternacaoDTO solicitacaoDeInternacaoDTO = solicitacaoDeInternacaoMapper.toDto(updatedSolicitacaoDeInternacao);
        ...
        SolicitacaoDeInternacao testSolicitacaoDeInternacao = solicitacaoDeInternacaoList.get(solicitacaoDeInternacaoList.size() - 1);
        assertThat(testSolicitacaoDeInternacao.getDataProvavelDaInternacao()).isEqualTo(UPDATED_DATA_PROVAVEL_DA_INTERNACAO);
        assertThat(testSolicitacaoDeInternacao.getDataProvavelDaCirurgia()).isEqualTo(UPDATED_DATA_PROVAVEL_DA_CIRURGIA);
        assertThat(testSolicitacaoDeInternacao.getPrioridade()).isEqualTo(UPDATED_PRIORIDADE);
        assertThat(testSolicitacaoDeInternacao.getPrincipaisSinaisESintomasClinicos()).isEqualTo(UPDATED_PRINCIPAIS_SINAIS_E_SINTOMAS_CLINICOS);
        assertThat(testSolicitacaoDeInternacao.getCondicoesQueJustificamInternacao()).isEqualTo(UPDATED_CONDICOES_QUE_JUSTIFICAM_INTERNACAO);
        assertThat(testSolicitacaoDeInternacao.getPrincipaisResultadosProvasDiagnosticas()).isEqualTo(UPDATED_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS);
    ...
    public void deleteSolicitacaoDeInternacao() throws Exception {
        ...
        // Get the solicitacaoDeInternacao
        restSolicitacaoDeInternacaoMockMvc.perform(delete("/api/solicitacao-de-internacaos/{id}", solicitacaoDeInternacao.getId())
        ...
        // Validate the SolicitacaoDeInternacao in Elasticsearch
        verify(mockSolicitacaoDeInternacaoSearchRepository, times(1)).deleteById(solicitacaoDeInternacao.getId());
    }
    ...
    public void searchSolicitacaoDeInternacao() throws Exception {
        // Initialize the database
        solicitacaoDeInternacaoRepository.saveAndFlush(solicitacaoDeInternacao);
        when(mockSolicitacaoDeInternacaoSearchRepository.search(queryStringQuery("id:" + solicitacaoDeInternacao.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(solicitacaoDeInternacao), PageRequest.of(0, 1), 1));
        // Search the solicitacaoDeInternacao
        restSolicitacaoDeInternacaoMockMvc.perform(get("/api/_search/solicitacao-de-internacaos?query=id:" + solicitacaoDeInternacao.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(solicitacaoDeInternacao.getId().intValue())))
            .andExpect(jsonPath("$.[*].dataProvavelDaInternacao").value(hasItem(DEFAULT_DATA_PROVAVEL_DA_INTERNACAO.toString())))
            .andExpect(jsonPath("$.[*].dataProvavelDaCirurgia").value(hasItem(DEFAULT_DATA_PROVAVEL_DA_CIRURGIA.toString())))
            .andExpect(jsonPath("$.[*].prioridade").value(hasItem(DEFAULT_PRIORIDADE.toString())))
            .andExpect(jsonPath("$.[*].principaisSinaisESintomasClinicos").value(hasItem(DEFAULT_PRINCIPAIS_SINAIS_E_SINTOMAS_CLINICOS)))
            .andExpect(jsonPath("$.[*].condicoesQueJustificamInternacao").value(hasItem(DEFAULT_CONDICOES_QUE_JUSTIFICAM_INTERNACAO)))
            .andExpect(jsonPath("$.[*].principaisResultadosProvasDiagnosticas").value(hasItem(DEFAULT_PRINCIPAIS_RESULTADOS_PROVAS_DIAGNOSTICAS)));
    }
