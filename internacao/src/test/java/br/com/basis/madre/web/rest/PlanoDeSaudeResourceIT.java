...
public class PlanoDeSaudeResourceIT {
    ...
    public void setup() {
        ...
        final PlanoDeSaudeResource planoDeSaudeResource = new PlanoDeSaudeResource(
            planoDeSaudeService);
    ...
    public void updatePlanoDeSaude() throws Exception {
        ...
        PlanoDeSaude updatedPlanoDeSaude = planoDeSaudeRepository.findById(planoDeSaude.getId())
            .get();
    ...
    public void searchPlanoDeSaude() throws Exception {
        // Initialize the database
        planoDeSaudeRepository.saveAndFlush(planoDeSaude);
        when(mockPlanoDeSaudeSearchRepository
            .search(queryStringQuery("id:" + planoDeSaude.getId()), PageRequest.of(0, 20)))
            .thenReturn(
                new PageImpl<>(Collections.singletonList(planoDeSaude), PageRequest.of(0, 1), 1));
        // Search the planoDeSaude
        restPlanoDeSaudeMockMvc
            .perform(get("/api/_search/plano-de-saudes?query=id:" + planoDeSaude.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(planoDeSaude.getId().intValue())))
            .andExpect(jsonPath("$.[*].nome").value(hasItem(DEFAULT_NOME)));
