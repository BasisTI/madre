...
/**
 * REST controller for managing {@link br.com.basis.madre.exames.domain.Recomendacao}.
 */
@RestController
@RequestMapping("/api")
public class RecomendacaoResource {
    ...
    public List<RecomendacaoDTO> getAllRecomendacaos() {
        log.debug("REST request to get all Recomendacaos");
    ...
    public List<RecomendacaoDTO> searchRecomendacaos(@RequestParam String query) {
        log.debug("REST request to search Recomendacaos for query {}", query);
        return recomendacaoService.search(query);
    }
}
