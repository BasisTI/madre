...
/**
 * REST controller for managing {@link br.com.basis.madre.madreexames.domain.ProjetoDePesquisa}.
 */
@RestController
@RequestMapping("/api")
public class ProjetoDePesquisaResource {
    ...
    public ResponseEntity<List<ProjetoDePesquisaDTO>> getAllProjetoDePesquisas(Pageable pageable) {
        log.debug("REST request to get a page of ProjetoDePesquisas");
        Page<ProjetoDePesquisaDTO> page = projetoDePesquisaService.findAll(pageable);
    ...
    public ResponseEntity<List<ProjetoDePesquisaDTO>> searchProjetoDePesquisas(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ProjetoDePesquisas for query {}", query);
        Page<ProjetoDePesquisaDTO> page = projetoDePesquisaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
