...
export class PacienteCartaoSusFormComponent implements OnInit{
    ...
    ngOnInit(): void {
        if(this.formGroup.get('cartaoSUS').get('justificativaId').value != null){
            this.justificativaService.find(this.formGroup.get('cartaoSUS').get('justificativaId').value).subscribe(res => {
        ...
        if(this.formGroup.get('cartaoSUS').get('motivoDoCadastroId').value != null){
            this.motivoDoCadastroService.find(this.formGroup.get('cartaoSUS').get('motivoDoCadastroId').value).subscribe(res => {
