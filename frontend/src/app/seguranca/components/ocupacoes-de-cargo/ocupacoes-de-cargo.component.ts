...
export class OcupacoesDeCargoComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.cargosService.getCargos().subscribe((response) => {
            this.cargos = response;
            this.cargos.unshift({ id: null, descricao: 'Selecione' });
        });
    }
    ...
    submit() {
        ...
        if (this.formOcupacoesDeCargo.value.id != 0) {
            this.ocupacoesDeCargoService
                .alterarOcupacoesDeCargo(ocupacoesDeCargo)
                .subscribe((e) => {
